...
export class NotificationProcessor extends WorkerHost {
  ...
  constructor(
    ...
    if (!publicKey || !privateKey) {
      throw new Error('VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY must be set — refusing to start with no push credentials.');
    }
    webpush.setVapidDetails('mailto:mohiiish.com@gmail.com', publicKey, privateKey);
  }
  ...
  private async handleRealtimeAlert(data: any) {
    ...
    this.logger.log(`Dispatching realtime alert: ${type} (room=${roomId}, user=${userId})`);
    ...
    if (type === 'suggestions-ready') {
      if (userId) {
        this.realtimeGateway.server.to(`user:${userId}`).emit('suggestions-ready', {
          success: true,
    ...
    if (type === 'WEEKLY_DIGEST') {
      ...
      const totalActionItems = summaries.reduce((acc, curr) => acc + (curr.actionItems?.length || 0), 0);
  ...
  private async sendPushIfOffline(userId: string, notification: { title: string; body: string; url: string }) {
  ...
  private async handleAppointmentNotification(appointment: any, title: string) {
    ...
    const eventName = title === 'New Appointment' ? 'newAppointment' : 'appointment:updated';
